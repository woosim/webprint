...
function WSEncoder(){
    ...
    this.setCodeTable = function(d){
        ...
        return "<codeTable>" + checkRegexp(d, /^(cp437|cp850|cp860|cp852|cp857|cp737|cp866|cp775|cp858|win1252|win1251|win1250|win1253|win1254|win1255|win1258|win1257|euc-kr|shift-jis)$/) + "</codeTable>";
    };
    ...
    this.setCharSpace = function(d){
        if(void 0 == d) throw Error("charSpace is undefined.");
        return "<charSpace>" + checkRange(d, 0, 255) + "</charSpace>";
    };
    
    this.setLineSpace = function(d){
        if(void 0 == d) throw Error("lineSpace is undefined.");
        return "<lineSpace>" + checkRange(d, 0, 255) + "</lineSpace>";
    };
    ...
    this.addTruncatedPDF417 = function (b) {
        var xmlElement="";
        if(void 0 != b) {
    ...
    this.addFeedToMark = function(){
        return "<feedToMark/>";
    };
    
    this.setPositionFromMark = function(d){
        return "<positionFromMark>" + checkRange(d, 1, 255) + "</positionFromMark>"; 
    };
    
    this.printData = function(m){
        if(m == void 0) throw Error("printData is undefined.");
        return "<printData>" + checkRegexp(m, /^(page|standard)$/) + "</printData>";
    };
    
    this.initPrinterStatus = function(){
        return "<initPrinterStatus/>";
    };
    
    this.addRawData = function(d){
        if(d == void 0) return Error("rawdata is undefined.");
        var bufferView = new Uint8Array(d);
        var binary = '';
        var len = d.byteLength;
        var xmlElement = "<rawData";
            
        for (var i = 0; i < len; i++) {
            binary += String.fromCharCode( bufferView[ i ] );
        }
        
        return  xmlElement+= " length=" + '"' + len + '">', 
            xmlElement+= window.btoa( binary ),
            xmlElement+= "</rawData>";
    };
